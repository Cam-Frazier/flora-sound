(() => {
  const calendarId =
    'c_c1586bd210f3ecb74b3f3c340a42b40d46270561d4a1c0fe7ade27b0d0c79a19@group.calendar.google.com';
  ...
  const getEndDate = event =>
    new Date(event.end.dateTime || `${event.end.date}T00:00:00`);
  ...
  function getTicketUrl(description = '') {
    ...
    const ticketAnchor = anchors.find(anchor =>
      /ticket/i.test(anchor.textContent || '')
    );

    if (ticketAnchor) {
      return ticketAnchor.href;
    }

    const plainText = doc.body.textContent || '';

    const labelled = plainText.match(
      /(?:tickets?|ticket link)\s*:\s*(https?:\/\/\S+)/i
    );
  ...
  function cleanDescription(description = '') {
    ...
    // Remove ticket links from the visible description.
    doc.querySelectorAll('a').forEach(anchor => {
      const text = anchor.textContent || '';

      if (/ticket/i.test(text)) {
        anchor.remove();
      } else {
    ...
    // Convert Google Calendar <br> tags to actual line breaks.
    doc.querySelectorAll('br').forEach(br => {
      br.replaceWith(document.createTextNode('\n'));
    });

    // Preserve breaks around block-level elements.
    doc.querySelectorAll('p, div, li').forEach(element => {
    ...
    return (doc.body.textContent || '')
      ...
      .filter(Boolean)
      .filter(line => {
        return !/^(?:tickets?|ticket link)\s*:?/i.test(line);
      });
  }
  ...
  function eventCard(event) {
    ...
    const detailsButton = event.htmlLink
      ? `
        <a
          class="text-link event-details"
          href="${escapeHtml(event.htmlLink)}"
          target="_blank"
          rel="noopener"
        >
          Event Details
      ...
      <article class="event-card">
        ...
        <div class="event-content">
          <p class="event-schedule">
            ${escapeHtml(formatSchedule(event))}
          </p>

          <h3>
            ${escapeHtml(event.summary || 'Flora Sound System Event')}
          </h3>
  ...
  async function loadEvents() {
    ...
    try {
      ...
      if (!events.length) {
        status.textContent =
          'New gatherings are taking root. Check back soon for upcoming dates.';
      ...
      status.textContent =
        'The events calendar could not be loaded right now. Please use “View Full Calendar” below.';
